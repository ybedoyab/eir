...
import { APP_ROUTES, APP_ROUTE_BUILDERS } from "@/config/app";
import {
  PATIENT_SAFE_RECOVERY_EVENTS,
  RECOVERY_EVENTS,
  RECOVERY_VIDEO_EVENTS,
  RECOVERY_VIDEO_TIMING,
...
function videoAction(pending: boolean, hasVideo: boolean): VideoAction {
  if (pending) return { icon: "refresh", label: "Generating…", spinning: true };
  if (hasVideo) return { icon: "refresh", label: "Regenerate video" };
  return { icon: "play", label: "Generate video" };
}
...
export default function PatientRecoveryPage() {
  ...
  const active = episodes.find((item) => !TERMINAL_RECOVERY_STATUSES.has(item.status));
  ...
  return (
    <>
        ...
        <section className="flex flex-col">
          ...
          {history.map((episode) => (
            ...
            >
              ...
              <Link
              href={APP_ROUTE_BUILDERS.recoveryDetails(episode.id)}
              className="eir-control focus-ink inline-flex min-h-11 items-center gap-2 px-2 text-[0.875rem] text-accent hover:bg-accent-tint hover:text-ink"
              >
                Details
      ...
      <section className="eir-panel on-raised flex flex-col gap-4 border-l-[3px] border-accent bg-raised px-8 py-7 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link href={APP_ROUTES.demo} className="shrink-0">
...
function RecoverySection({
  ...
  const checkin = [...events]
    .reverse()
    .find((item) => item.event_type === RECOVERY_EVENTS.patientResponded);
  ...
  const safeEvents = events.filter((item) => PATIENT_SAFE_RECOVERY_EVENTS.has(item.event_type));
  ...
  return (
    <section className="flex flex-col">
      ...
      <div className="flex flex-col gap-8">
        <div className="eir-panel on-tint flex flex-col overflow-hidden border border-accent/20 bg-accent-tint shadow-[0_10px_28px_rgb(22_75_130/0.08)]">
        ...
        <Link
          href={APP_ROUTE_BUILDERS.recoveryDetails(episode.id)}
          className="eir-control focus-ink inline-flex min-h-14 w-fit items-center gap-2.5 border border-accent px-6 text-[0.9375rem] font-medium text-accent hover:bg-accent-tint"
        >
...
function RecoveryVideoPanel({
  ...
  const latestVideoEvent = [...events]
    .reverse()
    .find((item) => RECOVERY_VIDEO_EVENTS.has(item.event_type));
  const pending = latestVideoEvent?.event_type === RECOVERY_EVENTS.videoRequested || requesting;
  const videoUrl =
    latestVideoEvent?.event_type === RECOVERY_EVENTS.videoReady
      ? String(latestVideoEvent.payload.video_url ?? "")
      : "";
  const failureReason =
    latestVideoEvent?.event_type === RECOVERY_EVENTS.videoFailed
      ? String(latestVideoEvent.payload.reason ?? "unknown error")
      : "";
  const action = videoAction(pending, Boolean(videoUrl));
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (Date.now() - startedAt > RECOVERY_VIDEO_TIMING.pollTimeoutMs) {
        window.clearInterval(timer);
        return;
      }
      onRefresh();
    }, RECOVERY_VIDEO_TIMING.pollIntervalMs);
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, ERROR_MESSAGES.video));
    } finally {
      setRequesting(false);
    }
  };
      ...
      <SectionHeader
        ...
        action={
          <Button variant="secondary" onClick={() => void handleRegenerate()} disabled={pending}>
            <span className={action.spinning ? "motion-safe:animate-spin" : undefined}>
      ...
      {videoUrl ? (
        <div className="flex justify-center">
          <video
            ...
            controls
            playsInline
            className="eir-panel aspect-[9/16] w-full max-w-[360px] border border-rule-strong bg-raised"
          />
        </div>
      ) : pending ? (
