...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        for name, width, height in (("desktop", 1440, 900), ("mobile", 390, 844)):
            ...
            page.get_by_role("link", name="Sign in by role", exact=True).click()
            page.wait_for_url("**/login")
            page.get_by_role("heading", name="Alex Rivera", exact=True).wait_for()
            shot("login")

            page.get_by_role("button", name="Continue as Alex Rivera", exact=True).click()
            page.wait_for_url("**/patient")
            page.get_by_text("Next appointment").wait_for()
            shot("patient-home")

            click_nav("Appointments", "/patient/appointments")
            shot("patient-appointments")

            click_nav("Ask EIR", "/patient/assistant")
            shot("patient-assistant")
            page.get_by_role("button", name="Show my appointments", exact=True).click()
            page.wait_for_timeout(6000)

            click_nav("Switch demo role", "/login")
            page.get_by_role("button", name="Continue as Dr. Maya Chen", exact=True).click()
            page.wait_for_url("**/clinician")
            page.get_by_text("Reviews waiting").wait_for()
            shot("clinician-home")
            click_nav("Schedule", "/clinician/schedule")
            shot("clinician-schedule")
            click_nav("Reviews", "/clinician/reviews")
            shot("clinician-reviews")

            click_nav("Switch demo role", "/login")
            page.get_by_role("button", name="Continue as Operations Admin", exact=True).click()
            page.wait_for_url("**/admin")
            page.get_by_text("Appointments today").wait_for()
            shot("admin-home")
            click_nav("Fleet", "/admin/fleet")
